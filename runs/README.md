...
| `race_v8/` | **The recruitment result (seeds 1–3).** Frozen stage-1.5 executor (`cont.pt`); the only learner is a ~4.5k-param route head. Conditions: `oracle`, `z_t`, `none` (whole wire zeroed — superseded as the headline floor). | 3 seeds, route-optimality: **oracle 0.990–0.996, z_t 0.986–1.000, none 0.488–0.518**; z_t hazard 0.00, executor obedience 1.000. The 5-seed anchored-`none` headline lives in `race_v8b/`. Two NaN post-mortems live in the trainer comments: reset-frame staleness and Bessel-corrected std on 1-element minibatches. |
...
| `diag/` | One-off diagnostics: exploration coverage sweeps, v7-oracle route-choice causal test, msg-sensitivity measurements, and the **WP1 corruption eval** (`eval_race_head_*`): frozen v8 heads, greedy argmax decisions, five wire modes (intact / zero_content / zero_all / shuffle / noise), equal-weight per-env episode accounting. | v7: oracle message WAS recruited but only gated advance-vs-balk, not corridor choice. WP1: **intact 1.000 on all 3 z_t seeds + 1 oracle; every corruption inside the coded ±0.10 band (0.41–0.56); executor obedience 1.000, worst hazard 0.88 steps. Coded checks pass** (hazard expectation was amended from crossings to balk-clips; estimator switched off a length-biased counter after the first run). |
